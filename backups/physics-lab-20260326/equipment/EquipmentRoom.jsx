...
export default function EquipmentRoom({ dispatch }) {
  ...
  const handleMouseDown = (e) => {
    // Only start drag select on left click on the background (not on items)
  ...
  const handleMouseMove = (e) => {
    ...
    setDragSelect(prev => ({ ...prev, currentX: x, currentY: y }));

    // Calculate selection box
    const minX = Math.min(dragSelect.startX, x);
    const maxX = Math.max(dragSelect.startX, x);
    const minY = Math.min(dragSelect.startY, y);
    const maxY = Math.max(dragSelect.startY, y);

    // Check which items are in the selection box
    const newSelected = [];
    for (const eq of EQUIPMENT) {
      const itemEl = itemRefs.current[eq.type];
      if (!itemEl) continue;
      ...
      if (itemRight > minX && itemLeft < maxX && itemBottom > minY && itemTop < maxY) {
        newSelected.push(eq.type);
      }
    }
  ...
  const handleMouseUp = () => {
    if (dragSelect) {
      // Mark that we just finished a drag, so onClick doesn't clear selection
  ...
  const panelOpen = selected.length > 0;
    ...
    <div style={{
      ...
      >
        ...
        <p style={{ fontSize: 12, color: "#555", margin: "0 0 12px" }}>
          Click to inspect • Ctrl/Cmd+click or drag to multi-select
        </p>
        ...
        <div style={{
          ...
          {EQUIPMENT.map(e => {
            ...
            return (
              <button
                key={e.type}
                ref={el => itemRefs.current[e.type] = el}
