...
// Random mass variation: up to ±5% from nominal
const varyMass = (nominalMass) => {
  const variation = 1 + (Math.random() * 0.1 - 0.05); // 0.95 to 1.05
  return nominalMass * variation;
};
...
export function reducer(state, action) {
  switch (action.type) {
    case "BRING": {
      ...
      // Apply mass variation to items with nominalMass (except massHanger which has exact calibrated weights)
      if (props.nominalMass !== undefined && action.eqType !== "massHanger") {
        props.mass = varyMass(props.nominalMass);
      }
      // massHanger uses totalMass which is exact (calibrated weights)
