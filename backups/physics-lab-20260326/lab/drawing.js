...
export function drawItem(ctx, item, all) {
  switch (item.type) {
    ...
    case "scale": drawScale(ctx, item, all); break;
    case "rodStand": drawRodStand(ctx, item); break;
  }
}
...
export function drawScale(ctx, sc, all) {
  const sx = sc.x, sy = sc.y;
  const sw = sc.props.width || 80, sh = sc.props.height || 40;

  // Find items on the scale platform
  const platformLeft = sx + 8;
  const platformRight = sx + sw - 8;
  const platformTop = sy;
  const platformBottom = sy + 10;

  let totalMassOnScale = 0;
  for (const item of all) {
    if (item.id === sc.id) continue;
    if (item.props.inTray) continue;
    if (item.type === "string" || item.type === "meterStick" || item.type === "protractor") continue;
    ...
    // Check if item's bottom is near scale platform and item is within platform bounds
    if (icx >= platformLeft && icx <= platformRight &&
        ib >= platformTop - 5 && ib <= platformBottom + 5) {
      // Get the item's mass
      if (item.type === "massHanger") {
        totalMassOnScale += item.props.totalMass || 0.1;
      } else if (item.props.mass !== undefined) {
        totalMassOnScale += item.props.mass;
      } else if (item.props.nominalMass !== undefined) {
        // Fallback to nominal if no actual mass set
        totalMassOnScale += item.props.nominalMass;
      }
    }
  }

  // Base
  const baseGr = ctx.createLinearGradient(sx, sy + sh - 15, sx, sy + sh);
  baseGr.addColorStop(0, "#e8e8f0");
  baseGr.addColorStop(1, "#c8c8d0");
  ctx.fillStyle = baseGr;
  ctx.beginPath();
  ctx.roundRect(sx, sy + sh - 15, sw, 15, 3);
  ctx.fill();
  ctx.strokeStyle = "#999";
  ctx.lineWidth = 0.5;
  ctx.stroke();
  ...
  ctx.beginPath();
  ctx.roundRect(sx + 6, sy, sw - 12, 10, 2);
  ctx.fill();
  ctx.strokeStyle = "#aaa";
  ctx.lineWidth = 0.5;
  ctx.stroke();
  ...
  // Support column
  ctx.fillStyle = "#bbb";
  ctx.fillRect(sx + sw / 2 - 4, sy + 10, 8, sh - 25);

  // Display
  ctx.fillStyle = "#1a2a1a";
  ctx.beginPath();
  ctx.roundRect(sx + sw / 2 - 18, sy + sh - 12, 36, 9, 2);
  ctx.fill();
  ctx.strokeStyle = "#333";
  ctx.lineWidth = 0.5;
  ctx.stroke();
  ...
  ctx.textAlign = "center";
  const displayMass = totalMassOnScale > 0 ? (totalMassOnScale * 1000).toFixed(1) : "0.0";
  ctx.fillText(`${displayMass}g`, sx + sw / 2, sy + sh - 5);
  ctx.textAlign = "start";

  // Buttons
  ctx.fillStyle = "#666";
  ctx.beginPath();
  ctx.arc(sx + 10, sy + sh - 8, 2.5, 0, Math.PI * 2);
  ctx.fill();
  ctx.beginPath();
  ctx.arc(sx + sw - 10, sy + sh - 8, 2.5, 0, Math.PI * 2);
  ctx.fill();
}
...
export function drawRodStand(ctx, rs) {
  ...
  ctx.beginPath();
  ctx.roundRect(rx, ry + rh - baseH, baseW, baseH, 2);
  ctx.fill();
  ctx.strokeStyle = "#333";
  ctx.lineWidth = 0.5;
  ctx.stroke();
  ...
  const rodW = 6;
  ctx.fillStyle = "#888";
  ctx.fillRect(rodX - rodW / 2, ry, rodW, rh - baseH);
  ctx.strokeStyle = "#666";
  ctx.lineWidth = 0.5;
  ctx.strokeRect(rodX - rodW / 2, ry, rodW, rh - baseH);
