...
export default function LabRoom({ labItems, strings, selectedId, dispatch, canvasRef, nbOpen, lqRef }) {
  ...
  return (
    ...
    >
      ...
      {trayDrag && (() => {
        // Calculate scale to match canvas rendering
        const viewW = zoomRegion ? (zoomRegion.maxX - zoomRegion.minX) : LOGICAL_W;
        const viewH = zoomRegion ? (zoomRegion.maxY - zoomRegion.minY) : LOGICAL_H;
        const previewScale = Math.min(displayW / viewW, displayH / viewH);

        const tw = trayDrag.props.width || 40;
        const th = trayDrag.props.height || 20;
        // Track needs extra height for feet
        const isTrack = trayDrag.type === "track";
        const footLen = 40; // BASE_FOOT_LENGTH from physics

        // Scaled dimensions for display (CSS pixels)
        const scaledW = tw * previewScale;
        const scaledH = (isTrack ? th + footLen : th) * previewScale;
        const scaledFootLen = footLen * previewScale;

        // For high-DPI displays, scale up canvas internal resolution
        const previewDpr = dpr;
        const cssW = scaledW + 20;
        const cssH = scaledH + 20;

        return (
        <div style={{
          position: "fixed",
          left: trayDrag.mouseX - scaledW / 2,
          top: trayDrag.mouseY - (th * previewScale) / 2,
          width: cssW,
          height: cssH,
          opacity: 0.5,
          pointerEvents: "none",
          ...
          <canvas
            ref={el => {
              if (!el) return;
              // Set internal resolution to DPR-scaled size
              el.width = cssW * previewDpr;
              el.height = cssH * previewDpr;
              const ctx = el.getContext("2d");
              ctx.globalAlpha = 0.6;
              // Scale all drawing by DPR for crisp rendering
              ctx.scale(previewDpr, previewDpr);

              if (trayDrag.type === "track") {
                // Draw track body at scaled size
                const sth = th * previewScale;
                const stw = scaledW;
                ctx.fillStyle = "#8a8a98";
                ctx.beginPath();
                ctx.roundRect(10, 10, stw, sth, 3 * previewScale);
                ctx.fill();
                ...
                // Draw feet
                const leftFootX = 10 + 20 * previewScale;
                const rightFootX = 10 + stw - 20 * previewScale;
                const footTop = 10 + sth;
                ctx.fillStyle = "#7a7a88";
                ctx.fillRect(leftFootX - 2.5 * previewScale, footTop, 5 * previewScale, scaledFootLen);
                ctx.fillRect(rightFootX - 2.5 * previewScale, footTop, 5 * previewScale, scaledFootLen);
                // Foot knobs
                ctx.fillStyle = "#aaa";
                ctx.beginPath();
                ctx.arc(leftFootX, footTop + scaledFootLen, 4 * previewScale, 0, Math.PI * 2);
                ctx.arc(rightFootX, footTop + scaledFootLen, 4 * previewScale, 0, Math.PI * 2);
                ctx.fill();
              } else if (trayDrag.type === "cart") {
                ctx.fillStyle = "#d03535";
                ctx.beginPath();
                ctx.roundRect(10, 10, scaledW, th * previewScale, 4 * previewScale);
                ctx.fill();
              } else if (trayDrag.type === "motionDetector") {
                ctx.fillStyle = "#344d66";
                ctx.beginPath();
                ctx.roundRect(10, 10, scaledW, th * previewScale, 5 * previewScale);
                ctx.fill();
              } else if (trayDrag.type === "bubbleLevel") {
                ctx.fillStyle = "#7a8a7a";
                ctx.beginPath();
                ctx.roundRect(10, 10, scaledW, th * previewScale, (th / 2) * previewScale);
                ctx.fill();
              } else if (trayDrag.type === "pulley") {
                ctx.fillStyle = "#888";
                ctx.beginPath();
                ctx.arc(10 + scaledW / 2, 10 + (th * previewScale) / 2, (tw / 2 - 2) * previewScale, 0, Math.PI * 2);
                ctx.fill();
              } else if (trayDrag.type === "string") {
                ctx.strokeStyle = "#bb9944";
                ctx.lineWidth = 2;
                ctx.beginPath();
                ctx.moveTo(10, 10 + (th * previewScale) / 2);
                ctx.lineTo(10 + scaledW, 10 + (th * previewScale) / 2);
                ctx.stroke();
              } else if (trayDrag.type === "massHanger") {
                ctx.fillStyle = "#6a6a78";
                ctx.fillRect(10 + 2 * previewScale, 10, scaledW - 4 * previewScale, th * previewScale);
              } else if (trayDrag.type === "meterStick") {
                ctx.fillStyle = "#ccaa55";
                ctx.beginPath();
                ctx.roundRect(10, 10, scaledW, th * previewScale, 2 * previewScale);
                ctx.fill();
              } else if (trayDrag.type === "scale") {
                // Base
                ctx.fillStyle = "#d8d8e0";
                ctx.beginPath();
                ctx.roundRect(10, 10 + th * previewScale * 0.6, scaledW, th * previewScale * 0.4, 2 * previewScale);
                ctx.fill();
                // Platform
                ctx.fillStyle = "#c0c0c8";
                ctx.beginPath();
                ctx.roundRect(10 + 5 * previewScale, 10, scaledW - 10 * previewScale, th * previewScale * 0.3, 2 * previewScale);
                ctx.fill();
              } else if (trayDrag.type === "rodStand") {
                // Base
                ctx.fillStyle = "#4a4a52";
                ctx.beginPath();
                ctx.roundRect(10, 10 + th * previewScale - 6 * previewScale, scaledW, 6 * previewScale, 2 * previewScale);
                ctx.fill();
                // Rod
                ctx.fillStyle = "#888";
                ctx.fillRect(10 + scaledW / 2 - 3 * previewScale, 10, 6 * previewScale, th * previewScale - 6 * previewScale);
              }
            }}
