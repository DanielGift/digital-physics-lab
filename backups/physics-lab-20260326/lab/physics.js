  ...
  for (const track of u) {
    ...
    if (!leftOverTable && !rightOverTable) {
      ...
      if (isStable) {
        ...
        // Increased sensitivity: more tilt per overhang for clearer visual feedback
        const tiltAmount = Math.min(maxStableTilt, (overhang / tw) * 1.2);
      ...
      // Land on surface below (only reset tilt if track was falling/tipping, not if stable)
      if (!isStable) {
        const leftSurface = findSurfaceBelow(leftFootX, track.y, u, tbl, track.id);
        const rightSurface = findSurfaceBelow(rightFootX, track.y, u, tbl, track.id);
        const leftFootY = track.y + 2 + leftFootLen;
      ...
      // NEW: Check if track BODY extends past table edge (even if feet are on table)
      // The track body extends from track.x to track.x + tw
      const bodyLeftEdge = track.x;
      const bodyRightEdge = track.x + tw;
      const leftBodyOverhang = Math.max(0, tbl.left - bodyLeftEdge);  // How much hangs off left
      const rightBodyOverhang = Math.max(0, bodyRightEdge - tbl.right); // How much hangs off right
      const hasBodyOverhang = leftBodyOverhang > 10 || rightBodyOverhang > 10;

      if (hasBodyOverhang && !oneFootHanging) {
        // Track body is hanging off, but both feet are on table
        // Calculate torque from the overhanging mass
        // Treat track as uniform density - overhang creates torque around nearest foot

        // Which side is overhanging more?
        const netOverhang = rightBodyOverhang - leftBodyOverhang;

        // The pivot point is the foot closest to the overhang
        // Calculate torque: proportional to overhang length squared (mass * lever arm)
        // Overhang mass fraction = overhang / tw
        // Lever arm ≈ overhang / 2 (center of overhanging section)
        // Torque ∝ (overhang/tw) * (overhang/2) = overhang^2 / (2*tw)

        const dominantOverhang = Math.max(leftBodyOverhang, rightBodyOverhang);
        const overhangFraction = dominantOverhang / tw;

        // The foot on the overhang side acts as pivot
        // Tilt amount based on torque - more overhang = more tilt
        // Use quadratic scaling so small overhangs have small effect
        const tiltAmount = Math.min(maxStableTilt, overhangFraction * overhangFraction * 8);

        // Tilt direction: positive = right side down, negative = left side down
        const tiltDir = netOverhang > 0 ? -1 : 1;  // Right overhang -> right side goes down -> negative tilt
        const targetTilt = tiltDir * tiltAmount;

        const currentTilt = track.props.initialTilt || 0;
        track.props.initialTilt = currentTilt + (targetTilt - currentTilt) * 0.12;

        // Position track so the supported (inner) foot touches the surface
        // The outer foot (near overhang) lifts up as track tilts
        const innerFootX = netOverhang > 0 ? leftFootX : rightFootX;
        const innerFootLen = netOverhang > 0 ? leftFootLen : rightFootLen;
        const innerFootLocalX = netOverhang > 0 ? 20 : (tw - 20);
        const innerSurfaceY = netOverhang > 0 ? leftTargetFootY : rightTargetFootY;

        const totalTiltAngle = tilt(track);
        const tiltOffset = Math.sin(totalTiltAngle) * (innerFootLocalX - tw / 2);
        const targetY = innerSurfaceY - 2 - innerFootLen + tiltOffset;
        track.y = track.y + (targetY - track.y) * 0.15;
        track.props.velocityY = 0;

        // Check if overhang is too extreme - track becomes unstable and tips
        // This happens when overhang exceeds ~40% of track length
        if (overhangFraction > 0.4) {
          // Track is unstable - start tipping over
          const angularVel = track.props.angularVelocity || 0;
          const angularAccel = G * 3.0 * (overhangFraction - 0.4);
          const newAngularVel = angularVel + angularAccel * dt;
          track.props.angularVelocity = newAngularVel;
          track.props.initialTilt = (track.props.initialTilt || 0) + tiltDir * newAngularVel * dt;
        }
      } else if (oneFootHanging) {
        ...
        if (centerOverSupport) {
          ...
          const totalTiltAngle = tilt(track);
          const tiltOffset = Math.sin(totalTiltAngle) * (supportedFootLocalX - tw / 2);
        ...
        // Both feet on similar height surfaces, no significant body overhang - settle normally
      ...
      const totalTiltAngle = footTilt + (track.props.initialTilt || 0);
      if (Math.abs(totalTiltAngle) > maxStableTilt) {
        const clampedTotal = Math.max(-maxStableTilt, Math.min(maxStableTilt, totalTiltAngle));
