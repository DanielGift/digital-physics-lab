...
const QuantumLearnerGrid = () => {
  ...
  const handleMouseMove = (e) => {
    ...
    const magnitude = Math.sqrt(gridPos.x ** 2 + gridPos.y ** 2);
    const isNormalized = Math.abs(magnitude - 1) <= MARGIN;
    const isEqualProb = Math.abs(Math.abs(gridPos.x) - Math.abs(gridPos.y)) <= MARGIN;
    
    if (isNormalized && isEqualProb) {
      setHint('✓ Perfect! Both normalization AND equal probabilities are satisfied!');
    } else if (isNormalized) {
      setHint('✓ Normalization is correct (|ψ|² = 1). Now make |x| = |y| for equal probabilities.');
    } else if (isEqualProb) {
      setHint('✓ Equal probabilities achieved (|x| = |y|). Now adjust magnitude to 1.');
    } else {
      setHint('');
    }
  };
  ...
  useEffect(() => {
    ...
    const isNormalized = Math.abs(magnitude - 1) <= MARGIN;
    const isEqualProb = Math.abs(Math.abs(pos.x) - Math.abs(pos.y)) <= MARGIN;
    
    // Draw highlight regions
    if (!selected) {
      // Draw normalization ring (magnitude = 1)
      if (isNormalized) {
        ctx.fillStyle = 'rgba(255, 255, 0, 0.15)';
        ctx.beginPath();
        const outerRadius = (1 + MARGIN) * SCALE;
        const innerRadius = (1 - MARGIN) * SCALE;
        ctx.arc(CANVAS_SIZE / 2, CANVAS_SIZE / 2, outerRadius, 0, 2 * Math.PI);
        ctx.arc(CANVAS_SIZE / 2, CANVAS_SIZE / 2, innerRadius, 0, 2 * Math.PI, true);
        ctx.fill();
      }
      
      // Draw equal probability X regions
      if (isEqualProb) {
        ctx.fillStyle = 'rgba(255, 255, 0, 0.15)';
        const thickness = MARGIN * SCALE;
        
        // Draw diagonal bands (y = x and y = -x)
        ctx.save();
        ctx.translate(CANVAS_SIZE / 2, CANVAS_SIZE / 2);
        
        // First diagonal (y = x)
        ctx.rotate(Math.PI / 4);
        ctx.fillRect(-CANVAS_SIZE, -thickness, CANVAS_SIZE * 2, thickness * 2);
        ctx.rotate(-Math.PI / 4);
      ...
      // Draw green regions where both conditions are met
      if (isNormalized && isEqualProb) {
        ...
        correctPositions.forEach(correctPos => {
          const canvasPos = gridToCanvas(correctPos.x, correctPos.y);
          ctx.beginPath();
          ctx.arc(canvasPos.x, canvasPos.y, MARGIN * SCALE * 1.5, 0, 2 * Math.PI);
          ctx.fill();
        });
      }
    }
  ...
  return (
    <div className="flex flex-col items-center gap-4 p-4 bg-gray-50 min-h-screen">
      <div className="max-w-3xl w-full bg-white rounded-lg shadow-lg p-4">
        ...
        {hint && !selected && (
          <div className="bg-yellow-50 border-2 border-yellow-400 p-3 rounded-lg mb-3 text-center">
...
export default QuantumLearnerGrid;